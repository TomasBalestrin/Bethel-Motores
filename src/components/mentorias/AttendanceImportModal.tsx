...
import { FileUp, Link2, Loader2, StickyNote, UserCheck } from "lucide-react";
...
async function submitAttendance(
  ...
  const response = await fetch(
    `/api/mentorias/${mentoriaId}/leads/attendance`,
    {
      method: "POST",
...
export function AttendanceImportModal({
  ...
  async function confirmImport() {
    ...
    setSubmitting(true);
    try {
      const response = await submitAttendance(mentoriaId, preview);
      setResult(response);
      if (response.matched > 0) {
        toast.success(
          response.matched === 1
            ? "1 lead marcado como compareceu"
            : `${response.matched} leads marcados como compareceu`,
          {
            description:
      ...
      toast.error("Falha ao marcar comparecimento", {
        description: error instanceof Error ? error.message : undefined,
      });
    } finally {
      setSubmitting(false);
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Importar lista de comparecimento — {mentoriaName}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-3 pt-3">
          {tab === "csv" ? (
            <div className="space-y-2">
              <Label htmlFor="att-csv">Arquivo CSV</Label>
              <input
                ref={fileInputRef}
                id="att-csv"
                type="file"
          ...
          {tab === "sheets" ? (
            <div className="space-y-2">
              <Label htmlFor="att-sheets">URL da planilha</Label>
              <Input
                id="att-sheets"
                type="url"
                value={sheetsUrl}
            ...
            <div className="space-y-2 rounded-md border border-border bg-muted/40 p-3 text-xs">
              <div className="flex flex-wrap items-center gap-3">
                <span className="font-semibold text-success">
                  {result.matched}{" "}
                  {result.matched === 1 ? "compareceu" : "compareceram"}
                </span>
          ...
          {!result ? (
            ...
            >
              {submitting ? (
                ...
                `Marcar comparecimento (${preview?.length ?? 0})`
              )}
            </Button>
          ) : null}
        </DialogFooter>
